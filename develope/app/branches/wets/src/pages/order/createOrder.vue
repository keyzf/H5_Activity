<template>
  <view>
    <!-- 地址 -->
    <navigator v-if="data[0].isFictitious == 0" :url="'/pages/address/address?source=1&isCustomAddress=' + data[0].isCustomAddress + '&cguid=' + data[0].cguid + '&customAddress=' + data[0].customAddress + '&postkind=' + this.postkind"
      class="address-section">
      <view class="order-content">
        <view class="cen">
          <view class="top"><text class="tip" v-show="addressData.defaultaddressstate == 1">默认</text><text>{{ addressData.address || '请选择收货地址' }}</text></view>
          <!-- <view class="address">合作路113号</view> -->
          <view class="name">{{ addressData.receivername + ' ' + addressData.receivertel }}</view>
        </view>
        <text class="yticon icon-you"></text>
      </view>
      <view class="tips">{{data.getFictitiousTip}}</view>
    ...
    <view class="address-section" v-else-if="daniurechargenumbertip != ''">
      <view class="order-content">
        <view class="cen">
          <view class="top" style="display: flex;">
            <text style="margin-right: 20rpx;">{{daniurechargenumbertip}}</text>
            <input style="flex: 1;" type="text" v-model="daniurechargenumber" :placeholder="'请输入'+daniurechargenumbertip" />
          </view>
        </view>
      </view>
      <view class="tips">{{data.getFictitiousTip}}</view>
    ...
    <view class="tips" v-if="openGroupbuyOneTeam == 0 && groupbuyOneTeamGuid == 0">友情提示:优惠券、活动和积分不能组合只用哦</view>
    <view class="yt-list" style="margin-top: 0;">
      <!-- 优惠明细 -->
      <view class="yt-list-cell b-b" v-if="openGroupbuyOneTeam == 0 && groupbuyOneTeamGuid == 0" @click="toggleMask('show')">
        <view class="cell-tit clamp">
          <text>优惠券</text>
          <text class="tip" v-show="price.discounttype == 2 && price.couponid != -1 && this.defultCoupon == price.couponid">已选推荐优惠</text>
        </view>
        <text class="cell-tip active">{{ price.discounttype == 2 ? price.couptitle : price.couponid == -1 ? '无' : '选择优惠券' }}</text>
        <text class="cell-more wanjia wanjia-gengduo-d"></text>
      </view>
      <view class="yt-list-cell b-b" v-if="openGroupbuyOneTeam == 0 && groupbuyOneTeamGuid == 0" @click="toggleSale('show')">
        <text class="cell-tit clamp">活动</text>
        <text class="cell-tip disabled">{{ price.discounttype == 3 ? price.saletitle : price.saleid == -1 ? '无' : '选择活动' }}</text>
        <text class="cell-more wanjia wanjia-gengduo-d"></text>
      </view>
      <view class="yt-list-cell b-b">
        <view class="cell-tit clamp">
          <text>余额</text>
          <text class="tits">{{ increment.extra_price }}</text>
        </view>
    ...
    <view class="yt-list">
      <view class="yt-list-cell b-b" v-if="openGroupbuyOneTeam == 0 && groupbuyOneTeamGuid == 0">
        <text class="tit">{{ increment.privilegepoint }}</text>
        <switch style="transform:scale(0.7)" :checked="privilegepointState" :disabled="increment.privilegepointvalue == 0"
          color="#fa436a" @click="switchClick('privilegepoint')" />
      </view>
      <view class="yt-list-cell b-b" v-if="openGroupbuyOneTeam == 0 && groupbuyOneTeamGuid == 0">
        <text class="tit">{{ increment.redpacketmoneyinfo }}</text>
        <switch style="transform:scale(0.7)" :checked="redpacketState" :disabled="increment.redpacketmoney == 0" color="#fa436a"
          @click="switchClick('redpacket')" />
      </view>
      <!-- <view class="yt-list-cell b-b">
                <text class="tit">{{ increment.extra_price }}</text>
                <switch style="transform:scale(0.7)" :checked="extraState" :disabled="increment.extra_value == 0" color="#fa436a" @click="switchClick('extra')" />
            </view> -->
      <!-- 金额明细 -->
      <view class="yt-list-cell b-b">
    ...
    methods: {
      ...
      getGoodsInfo() {
        ...
        if (this.type == 'cart') {
          ...
          data.keywords = this.keywords[0];
          data.productid = this.ids[0];
          data.addressid = this.addressData.addressid;
          data.couponid = this.price.couponid;
          this.groupbuyactivityguid ? data.groupbuyactivityguid = this.groupbuyactivityguid : '';
          this.takegroupbuyteamguid ? data.takegroupbuyteamguid = this.takegroupbuyteamguid : '';
      ...
      getPrice() {
        ...
        let url = this.type == 'cart' ? 'shoppingcart/selectgearstepfix' : 'shoppingcart/directselectgearstepfix';
        let data = {};
        if (this.type == 'cart') {
          data = {
          ...
          data = {
            ...
            bonuspoint: this.privilegepointState ? this.increment.privilegepointvalue : '0',
            saleid: this.price.discounttype == 3 ? this.price.saleid : -1,
            groupbuyactivityguid: this.groupbuyactivityguid,
      ...
      switchClick(type) {
        if (type === 'extra') {
          ...
          if (this.increment.redpacketmoney == 0) return;
          this.redpacketState = !this.redpacketState;
          if (this.redpacketState) {
            this.price.discounttype = 0;
            this.selectIncrement(type);
          }
        } else if (type === 'privilegepoint') {
          if (this.increment.privilegepointvalue == 0) return;
      ...
      createOrder() {
        ...
        this.idcard.id_number ? data.idnumber = this.idcard.id_number : '';
          ...
          if (res.data.code == 0) {
            ...
            if (this.price.price_usemealcard == '¥0.00') {
              ...
              if (data.type == 1) {
                ...
                url = '/pages/money/pay?ordernum=' + res.data.result.data.ordernum + '&price=' + data.real_price;
              }
            }
            uni.redirectTo({
              url
