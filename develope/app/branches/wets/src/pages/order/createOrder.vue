<template>
    <view>
        <!-- 地址 -->
        <navigator v-if="data[0].isFictitious == 0" :url="'/pages/address/address?source=1&isCustomAddress=' + data[0].isCustomAddress + '&cguid=' + data[0].cguid + '&customAddress=' + data[0].customAddress + '&postkind=' + this.postkind" class="address-section">
        ...
        <view class="tips">友情提示:优惠券、活动和积分不能组合只用哦</view>
        <view class="yt-list" style="margin-top: 0;">
            ...
            <view class="yt-list-cell b-b" @click="toggleSale('show')">
...
export default {
    ...
    data() {
        return {
			...
			postkind: '',
            data: [{cguid: 0, customAddress: 1, isCustomAddress: 1}],
            ids: [],
            numbers: [],
            keywords: [],
            skuid: 0,
    ...
    methods: {
                    ...
                    if (this.addressData.addressid == -3) {
                        uni.showModal({
                            ...
                            success: res => {
                                ...
                                if (res.confirm) {
                                    uni.navigateTo({
                                        url: '/pages/address/address?source=1&isCustomAddress=' + this.data[0].isCustomAddress + '&cguid=' + this.data[0].cguid + '&customAddress=' + this.data[0].customAddress + '&postkind=' + this.postkind
                                    });
                                }
                            }
                        });
        ...
        // 获取活动列表
        getSaleList() {
            let url = this.type == 'cart' ? 'shoppingcart/getsaleactivitylist_checkout' : 'shoppingcart/getsaleactivitylist_deccheckout';
            let data = {
                shopids: this.ids,
                skuid: this.skuid || '',
        ...
        createOrder() {
            ...
            data.idnumber = this.idcard.id_number;
              data.daniurechargenumber = this.daniurechargenumber;
        ...
        getproductinfoforcity() {
            this.$ajax.get('shoppingcart/getproductinfoforcity', { productids: this.tip.isaddresstipforcityproductids }).then(res => {
                if (res.data.code == 0) {
                    let data = res.data.result.data;
        ...
        startOrder(productids) {
            this.$ajax.get('order/startingBuylimitInNearFuture', {productids: productids}).then(res => {
