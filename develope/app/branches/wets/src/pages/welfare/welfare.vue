<template>
  <view class="content">
    ...
    <view class="navigateBar positionScrollX" :style="{ height: stateBarHeight + 'rpx', 'background-color': navBarBgColor }">
    ...
    <view class="topRed">
      ...
      <view class="moneyBlock">
        ...
        <view class="blockRight"><view class="middleRight" @click="clickDuiHuan()">福利券兑换</view></view>
      </view>
    ...
    <block v-if="isStart">
        ...
        <view class="hasSelectTop">
          ...
          <view class="hasSelectContent">
            <view class="hasSelectContentLeft">
              <block v-for="(item, index) in hasSelectedData.piclist" :key="index"><image :src="item.url" mode="scaleToFill"></image></block>
            </view>
      ...
      <!-- 套餐商品列表 -->
      <view class="taoCanBlock">
        <!-- todozcc到时候加套餐商品列表的数据 -->
        <!-- <view class="singleTaoCan" v-for="(subItem,subIndex) in (goodsList[1] || []).list" :key="subIndex">
					<view class="taoCanTop">
						...
						<view class="taoCanTopRight">
							...
							<image class="cartImage" :src="subItem.isSoldout == 1 ? '../../static/index/cartGray.png' : '../../static/index/cart.png'" mode="widthFix" :data-skuid="subItem.skuid" :data-isSoldout="subItem.isSoldout" @click="addToCart"></image>
						</view>
					</view>
					<view v-if="(subItem.detailList || []).length < 4">
						<block v-for="(singleGood,singleGoodIndex) in subItem.detailList" :key="singleGoodIndex">
							<view class="goodBlockTaoCanSingle" @click="clickGoodDetail(singleGood.id)">
								...
								<image v-if="(singleGood.videourl || '').length > 0" class="taoCanGoodVedio" src="../../static/index/taoCanGoodVedio.png" mode="scaleToFill" @click.prevent="taoCanGoodVedio(singleGood.videourl)"></image>
							</view>
						</block>
					</view>
					<view v-else class="goodBlockTaoCanMulti">
						<view class="taoCanMultiSingleBlock" v-for="(goodPic,goodPicIndex) in subItem.detailList" :key="goodPicIndex">
							...
							<image v-if="(goodPic.videourl || '').length > 0" class="taoCanGoodVedio" src="../../static/index/taoCanGoodVedio.png" mode="scaleToFill" @click.prevent="taoCanGoodVedio(goodPic.videourl)"></image>
						</view>
					</view>
				</view> -->
      </view>
      ...
      <!-- 商品类型View -->
      <scroll-view id="goodsType" class="scroll-x positionScrollX" scroll-x="true" show-scrollbar="false" :scroll-left="scrollLeft">
        <view
          class="goodsTypeBlock"
          v-for="(item, index) in goodsTypeList"
          :key="item.id"
          :id="item.id"
          @click="clickGoodsType(item.id, index)"
      ...
      <!-- 商品模块 4种展示形式 -->
      <swiper :current="tabIndex" class="goodsList" :style="{ height: 'calc(100% - ' + (stateBarHeight + 300) + 'rpx)' }" style="flex: 1;" :duration="300" @change="ontabchange">
        <swiper-item class="goodTypeBlock" v-for="(item, index) in goodsList" :key="index">
          <!-- <view ref='goodsTest' id="goodsTest"> -->
          <scroll-view class="scroll-v" v-bind:style="{ height: 'calc(100% - 0rpx)' }" enableBackToTop="true" :scroll-y="allowScrollY" @scrolltolower="loadMore()">
              ...
              <view class="singleTaoCan" v-for="(subItem, subIndex) in item.list" :key="subIndex">
                <view class="taoCanTop">
                  ...
                  <view class="taoCanTopRight">
                    ...
                    <image
                      class="cartImage"
                      :src="subItem.isSoldout == 1 ? '../../static/index/cartGray.png' : '../../static/index/cart.png'"
                      mode="widthFix"
                      :data-skuid="subItem.skuid"
                      :data-isSoldout="subItem.isSoldout"
                      @click="addToCart"
                    ></image>
                  </view>
                </view>
                  ...
                  <block v-for="(singleGood, singleGoodIndex) in subItem.detailList" :key="singleGoodIndex">
                    <view class="goodBlockTaoCanSingle" @click="clickGoodDetail(singleGood.id)">
                      ...
                      <view class="taoCanGoodDetail">
                        <view class="taoCanGoodTitle">{{ singleGood.name }}</view>
                        <view class="taoCanTeSe">
                          <view class="taoCanTeSeSingle">{{ singleGood.feature_short }}</view>
                        </view>
                        <view class="pinPai">{{ singleGood.brand }}</view>
                        <view class="guiGe">{{ singleGood.attrs }}</view>
                      </view>
                      <image
                        v-if="(singleGood.videourl || '').length > 0"
                        class="taoCanGoodVedio"
                        src="../../static/index/taoCanGoodVedio.png"
                        mode="scaleToFill"
                        @click.prevent="taoCanGoodVedio(singleGood.videourl)"
                      ></image>
                    </view>
                  </block>
                </view>
                <view v-else class="goodBlockTaoCanMulti">
                  <view class="taoCanMultiSingleBlock" v-for="(goodPic, goodPicIndex) in subItem.detailList" :key="goodPicIndex">
                    <image class="goodBlockTaoCanMultiPic" :src="goodPic.picurl" mode="scaleToFill" @click="clickGoodDetail(goodPic.id)"></image>
                    <image
                      v-if="(goodPic.videourl || '').length > 0"
                      class="taoCanGoodVedio"
                      src="../../static/index/taoCanGoodVedio.png"
                      mode="scaleToFill"
                      @click.prevent="taoCanGoodVedio(goodPic.videourl)"
                    ></image>
                  </view>
                </view>
              </view>
            ...
            <block v-else-if="item.type == 2">
              <view class="goodBlockRowTwo">
                <view class="singleGood" v-for="(subItem, subIndex) in item.list" :key="subIndex">
                  ...
                  <image
                    class="taoCanGoodVedio"
                    v-if="(((subItem.piclist || [])[0] || {}).videourl || '').length > 0"
                    src="../../static/index/taoCanGoodVedio.png"
                    mode="scaleToFill"
                    @click.prevent="taoCanGoodVedio((subItem.piclist || [])[0].videourl)"
                  ></image>
                  ...
                  <view class="goodBottom">
                    ...
                    <image
                      ...
                      :src="subItem.isSoldout == 1 ? '../../static/index/cartGray.png' : '../../static/index/cart.png'"
                      mode="widthFix"
                      @click="chooseGoodGuiGe(subItem.productId, subItem.isSoldout)"
                    ></image>
                    <!-- <view v-else class="chooseGuiGe" :style="{'background-color':subItem.isSoldout == 1 ? '#bebcbf' : '#ee3847'}" @click="chooseGoodGuiGe(subItem.productId,subItem.isSoldout)">选规格</view> -->
                  </view>
                </view>
              </view>
            </block>
            ...
            <!-- 底部刷新 -->
            <view class="loading-more" v-if="currentList[index].length >= pageLimit" v-on:click="loadMore()"><text class="loading-more-text">加载更多商品</text></view>
            <view class="loading-more" v-else-if="goodsList[index].list.length > 7"><text class="loading-more-text">没有更多商品了～</text></view>
          </scroll-view>
          <!-- </view> -->
        </swiper-item>
      </swiper>
      ...
      <!-- 商品选择规格的弹出框 -->
      <uni-popup ref="chooseGoodGuiGe" type="center" @change="popupGoodGuiGeChange">
        <view class="guiGePopup">
          <view class="goodName">{{ attributesData.productname }}</view>
          <view class="attrRow" v-for="(item, index) in attributesData.selectinfo" :key="index">
            <view class="attrName">{{ item.attributetitle }}</view>
            <view class="attrDetailRow">
              <block v-for="(subItem, subIndex) in item.attribute" :key="subIndex">
                <view
                  class="attributes-select-btn"
                  :class="[selectedIndexArr[index] == subIndex ? 'attributes-selected-btn' : '', enabledAttributes.indexOf(subItem.id) == -1 ? 'attributes-noSelected-btn' : '']"
                  @click="clilckAttr"
                  :data-outindex="index"
                  :data-innerindex="subIndex"
                  :data-id="subItem.id"
                  :data-outname="item.attributetitle"
                  :data-innername="subItem.name"
                >
                  {{ subItem.name }}
                </view>
              </block>
          ...
          <view class="attrBottom">
            <view class="attrPriceBlock">
              <view class="attrPrice">
                <text :style="{ 'font-size': '25rpx' }">{{ (attributesData.newprice || '').substring(0, 1) }}{{ ' ' }}</text>
                {{ (attributesData.newprice || '').substring(1, (attributesData.newprice || '').length) }}
              </view>
              <!-- <view class="attrOldPrice">¥500.00</view> -->
            </view>
            <view class="attrCart" @click="addAttrToCart">
              <image class="attrCartImage" src="../../static/index/attrAddCart.png" mode="scaleToFill"></image>
              <view class="attrCartText">加入购物车</view>
            </view>
          </view>
        </view>
        <view class="guiGeCloseView"><image class="guiGeClose" @click="closeGuiGeView" src="../../static/index/guiGeClose.png" mode="widthFix"></image></view>
      </uni-popup>

      <!-- 选购攻略 -->
      <block v-if="showGongLve"><drag-button :isDock="true" :existTabBar="true" @btnClick="clickGongLve()" /></block>
      ...
      <uni-popup ref="showcartList" :type="popupType" @change="popupCartChange">
        <view class="cartListView" @touchmove.prevent="" @click.prevent="">
          <view class="tipView" v-if="(selectedCartData.freighttips || '').length > 0">
            <text>{{ selectedCartData.freighttips }}</text>
          </view>
            ...
            <view class="shopRow" v-for="(shopItem, shopIndex) in selectedCartData.skuinfoList" :key="shopIndex" :style="{ 'margin-top': shopIndex == 0 ? '-30rpx' : '0' }">
              ...
              <view class="goodRow" v-for="(goodItem, goodIndex) in shopItem.skulist" :key="goodIndex">
                <view class="goodLeft">
                  ...
                  <image class="goodLeftSoldOutImage" v-if="goodItem.state == 0" src="../../static/index/soldOut.png" mode="scaleToFill"></image>
                </view>
                <view class="goodTotalRight">
                  <view class="goodTitle">{{ goodItem.shortname }}</view>
                  <view class="goodTitleBottom">
                    <view class="goodSubLeft">
                      <view class="goodSubTitle">
                        <view class="goodAttrbutes">{{ goodItem.attributeshow }}</view>
                        <view class="goodTip">{{ goodItem.delivery_tips }}</view>
                      </view>
                      <view class="goodPrice" :style="{ 'margin-top': (goodItem.delivery_tips || '').length > 0 ? '0' : '30rpx' }">
                        ¥ {{ goodItem.newprice }}
                        <text class="goodFreight">{{ goodItem.freighttips }}</text>
                      </view>
                    </view>
    ...
    <block v-else>
      ...
      <view class="emptyView">
        ...
        <view class="emptyText1">
          <text>{{ expectingData.text }}</text>
        </view>
    ...
    <uni-popup ref="showvedioPart" :type="popupType" @change="popupVedioPart">
      <view class="vedioPart"><video id="myVideo" :src="vedioUrl" @error="videoErrorCallback" autoplay controls object-fit="fill"></video></view>
...
const AttributesUtil = require('./utils/attributes');
...
export default {
  ...
  data() {
    return {
      ...
      // 多规格的属性弹出框
      attributesData: {}, //商品规格的总数据
      enabledAttributes: [],
      selectedIndexArr: [],
      selectedAttributeIDsArr: [],
      selectedAttributeNameArr: [],
      unSelectedOuterAttributesArr: []
    };
  },
  onLoad() {
    var that = this;
  ...
  onPageScroll: function(e) {
    ...
    // 	return;
    // }
    // if (this.scrollY>=this.goodsPartTopArr[0]-(this.globaldata.statusBarHeight+55)) {
    // 	this.allowScrollY = true
    // }
    // else {
    // 	this.allowScrollY = false
    // }
    if (this.scrollY >= this.goodsPartTopArr[1].top - this.goodsPartTopArr[0].height - 25) {
      this.allowScrollY = true;
    } else {
      this.allowScrollY = false;
    }
  ...
  methods: {
          ...
          for (var i = 0; i < data.length; i++) {
            ...
            if (dic.flag == 1) {
              if (dic.code == 'slides') {
                //顶部红色
                this.slides();
              } else if (dic.code == 'myaccount') {
                //余额
                this.accountFlag = true;
                this.account();
              } else if (dic.code == 'headline') {
                //头条
                this.headline();
              } else if (dic.code == 'tabs') {
                //商品类型
                this.mytabs();
              } else if (dic.code == 'myhelp') {
                //选购攻略或福利说明
                this.showGongLve = true;
              } else if (dic.code == 'myorder') {
                //订单及物流
                this.myorder();
              } else if (dic.code == 'expecting') {
                //未开始的敬请期待
                this.expecting();
              }
            }
    ...
    productListByTabID(index) {
      ...
      API.productListByTabid({
        ...
        .then(res => {
          ...
          // 快速切换有重复商品的情况，先这样解决吧
          if (resData.length > 0 && (this.goodsList[index].list || []).indexOf(resData[0]) != -1) {
            return;
          }
    ...
    firstInCartInfo() {
      API.firstInCartInfo({})
        .then(res => {
          ...
          this.skuinfo = JSON.parse((res.data.result.data || {}).skus || JSON.stringify([]));
          this.skuinfoSubmit = JSON.parse(JSON.stringify(this.skuinfo));

          this.chooseGoodNumber = 0;
          for (let i = 0; i < this.skuinfo.length; i++) {
            this.chooseGoodNumber = this.chooseGoodNumber + parseInt(this.skuinfo[i].number);
          }
    ...
    initShoppingCart(number, isClear) {
      API.shoppingCart({
        skuinfo: isClear == 1 ? JSON.stringify([]) : JSON.stringify(this.skuinfo)
      })
        .then(res => {
          // 解除防连击
          ...
          if (res.data.code != 0) {
            this.skuinfo = JSON.parse(JSON.stringify(this.skuinfoSubmit));
            uni.showModal({
              title: '提示',
              content: res.data.msg,
              showCancel: false
    ...
    checkOut(orderNumber) {
      if (this.isapp()) {
        ...
        uni.navigateTo({
          url:'/pages/order/createOrder?welfareid=' +this.globaldata.welfareid +'&type=cart&skuinfo=' + JSON.stringify(this.skuinfoSubmit)
        });
      }
    },
    // 计算位置
    jiSuanGoodsTopHeight() {
      ...
      setTimeout(() => {
        uni
          .createSelectorQuery()
          .in(this)
          .selectAll('.positionScrollX')
          .boundingClientRect(rects => {
            for (var i = 0; i < rects.length; i++) {
              var dic = rects[i];
              this.goodsPartTopArr.push(dic);
            }
          })
          .exec();
      }, 500);
    ...
    clickGoodDetail(productID) {
      if (this.isapp()) {
        ...
        uni.navigateTo({
          url: '/pages/product/product?tabid=' + this.goodsTypeId + '&productid=' + productID
        });
      }
    },
    // 选择规格
    chooseGoodGuiGe(productID, soldOut) {
      if (soldOut == 1) {
        return;
      }
      ...
      this.selectedAttributeIDsArr = [];
      this.selectedAttributeNameArr = [];
      this.unSelectedOuterAttributesArr = [];
      ...
      API.getGoodGuiGe({
        ...
        .then(res => {
          ...
          this.$refs['chooseGoodGuiGe'].open();

          this.attributesData = res.data.result.data;
          // 属性数据清洗
          AttributesUtil.reformAllSkuInfoFac(this.attributesData);
          this.enabledAttributes = AttributesUtil.enabledAttributesFac(AttributesUtil.initSelectAttrs);

          // 初始化选择属性id的数组,默认选中单属性的
          const selectInfo = this.attributesData.selectinfo;
          this.selectedIndexArr = [];
          this.selectedAttributeNameArr = [];
          this.unSelectedOuterAttributesArr = [];
          for (var i = 0; i < selectInfo.length; i++) {
            const attributeId = selectInfo[i].attribute[0].id;
            if (selectInfo[i].attribute.length == 1 && this.enabledAttributes.indexOf(attributeId) != -1) {
              this.selectedIndexArr.push('0');
              this.selectedAttributeIDsArr.push(attributeId);
              this.selectedAttributeNameArr.push(selectInfo[i].attribute[0].name);

              // 判断其他属性是否可以选择
              this.enabledAttributes = AttributesUtil.enabledAttributesFac(this.selectedAttributeIDsArr);
            } else {
              this.selectedIndexArr.push('-1');
              this.selectedAttributeIDsArr.push('');
              this.unSelectedOuterAttributesArr.push(selectInfo[i].attributetitle);
              this.selectedAttributeNameArr.push('');
            }
          }

          // 有默认的选择属性，更新价格和库存（attributesData数据）
          const skuKeysStr = AttributesUtil.reformSkuKeyStringFac(this.selectedAttributeIDsArr);
          const selectAttributesInfo = AttributesUtil.skuData[skuKeysStr];
          this.attributesData['newprice'] = selectAttributesInfo.prices[0];
          this.attributesData['stock'] = selectAttributesInfo.stock;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    // 关闭规格弹窗
    closeGuiGeView() {
      this.$refs['chooseGoodGuiGe'].close();
    },
    // 监听规格弹窗是否关闭
    popupGoodGuiGeChange(res) {
      // res.show
    },
    // 选择商品属性
    clilckAttr(e) {
      const attributeID = e.currentTarget.dataset.id;
      // 不可点击的属性，点击直接返回
      if (this.enabledAttributes.indexOf(parseInt(attributeID)) == -1) {
        return;
      }
      const attributeName = e.currentTarget.dataset.innername;
      const outIndex = e.currentTarget.dataset.outindex;
      const outerAttr = e.currentTarget.dataset.outname;

      if (this.selectedAttributeIDsArr.indexOf(attributeID) == -1) {
        // 更新选择属性的ID
        this.selectedAttributeIDsArr[outIndex] = attributeID;
        // 更新选择属性的索引
        this.selectedIndexArr[outIndex] = e.currentTarget.dataset.innerindex;
        // 更新选择属性的名称
        this.selectedAttributeNameArr[outIndex] = attributeName;
        // 更新未选择外层标题
        let outerAttrIndex = this.unSelectedOuterAttributesArr.indexOf(outerAttr);
        if (outerAttrIndex > -1) {
          this.unSelectedOuterAttributesArr.splice(outerAttrIndex, 1);
        }
      } else {
        // 更新选择属性的ID
        this.selectedAttributeIDsArr[outIndex] = '';
        // 更新选择属性的索引
        this.selectedIndexArr[outIndex] = -1;
        this.selectedAttributeNameArr[outIndex] = '';
        this.unSelectedOuterAttributesArr.push(outerAttr);
      }

      // 判断其他属性是否可以选择
      this.enabledAttributes = AttributesUtil.enabledAttributesFac(this.selectedAttributeIDsArr);

      // 选择属性，更新价格和库存（attributesData数据）
      const skuKeysStr = AttributesUtil.reformSkuKeyStringFac(this.selectedAttributeIDsArr);
      const selectAttributesInfo = AttributesUtil.skuData[skuKeysStr];
      this.attributesData['newprice'] = selectAttributesInfo.prices[0];
      this.attributesData['stock'] = selectAttributesInfo.stock;
    ...
    addAttrToCart(e) {
      // 是否有未选择的属性
      if (this.selectedAttributeIDsArr.indexOf('') > -1) {
        uni.showModal({
          title: '提示',
          content: '请选择' + this.attributesData.selectinfo[this.selectedAttributeIDsArr.indexOf('')].attributetitle,
          showCancel: false
        });
        return;
      }
      ...
      let skuid = (selectAttributesInfo.skuIds[0] || '') + '';
    ...
    clickGoodsType(type, index) {
      ...
      // 滑动操作
      // var top = this.goodsPartTopArr[index] - (index == 0 ? 115 : 107);
      // document.body.scrollTop=top;
      // this.$nextTick(() => {
      // 	// document.documentElement.scrollTop=400
      // 	// this.$refs['goodsTest'].scrollTop = 200;
      // 	// this.$refs.goodsTest.scrollTop = (0,100);
      // 	// document.getElementById('goodsTest').scrollIntoView()
      // 	var top = this.goodsPartTopArr[0] - 120;
      // 	document.body.scrollTop=top;
      // })
  ...
  .guiGePopup {
    ...
    .attrRow {
      ...
      .attrDetailRow {
        ...
        .attributes-selected-btn {
          color: #ee3847;
          background-color: #fff5f4;
          border: 1px solid #ee3847;
        }

        .attributes-noSelected-btn {
          color: #9d9d9e;
