<template>
	<view class="content">
        <view class="address-section" @click="editadds()">
            <view>获取微信收货地址</view>
		...
		<view class="row b-b" v-show="type >= 3">
			...
			<input @click="clickAddressName()" class="input" type="text" disabled="true" v-model="addressData.districtname" placeholder="省市区县、乡镇等" placeholder-class="placeholder" />
		</view>
		<view class="row b-b" v-show="type >= 3"> 
...
<script>
	...
	export default {
		...
		methods: {
            // 获取地址类型
            getAddressType() {
                this.$ajax.get('shoppingcart/getAddressTypeByAid', {addressid: this.addressData.addressid}).then(res => {
                    console.log(res);
                    if (res.data.code == 0) {
			...
			editAddress() {
                this.$ajax.get('shoppingcart/saveaddress', {
                    ...
                    //welfareid: this.addressData.welfareid,
                    //alternatetel: this.addressData.alternatetel,
			...
			onCityClick(val){
				...
				if (val.town.value == 0) {
					...
					this.addressData.districtname = val.province.label + ',' + val.city.label + ',' + val.county.label + ',' + val.town.label;
				}
				// 区县id
            ...
            // 获取地址列表
            pickerClick() {
                this.$ajax.get('shoppingcart/workshoplist', {addresstype: this.type}).then(res => {
                    if (res.data.code == 0) {
                        let _this = this;
                        _this.addressArray = [];
            ...
            editadds(){
                this.$wx.editadd().then(res => {
                    console.log(res)
                    this.addressData.receivername = res.userName; // 收货人姓名
                    this.addressData.districtname = res.provinceName+","+res.cityName+","+res.countryName;
                    this.addressData.address = res.detailInfo; // 详细收货地址信息
                    this.addressData.receivertel = res.telNumber; // 收货人手机号码
                });
            }
		}
	}
