<template>
	<view class="conbox">
		...
		<view class="container" :class="{x:isapp()}">
			...
			<view class="btn" @click="gomain"></view>
			<view class="typelist">
			...
			<view class="prizecon">
				<view class="guize" v-show="types==0">
					...
					<view class="g_item">
						1.活动期间用户每天登录商城，可获得1次抽奖机会；
					</view>
					<view class="g_item">
						2.活动期间用户每天分享大转盘活动，可获得1次抽奖机会；
					</view>
					<view class="g_item">
						3.活动期间用户每次下单成功，可获得2次抽奖机会；
					</view>
				</view>
		...
		<uni-popup ref="guize" type="center" class="ysguize">
			...
			<view class="guize">
				...
				<view class="g_item">
					1.活动时间：2020年6月23日——2020年6月30日；
				</view>
				<view class="g_item">
					2.抽到的奖券可在【我的】→【我的钱包】→【优惠券】中查看，奖券也可转赠他人；
				</view>
				<view class="g_item">
					3.请在奖券标明的有效期限内使用您的奖券；
				</view>
				<view class="g_item">
					4.奖券的适用范围为商城自营商品，但特殊的商品（虚拟商品）除外；
				</view>
				<view class="g_item">
	...
	export default {
		...
		data() {
			return {
				...
				show: false,
				number: 0,
				tip: ['今日抽奖次数已用完，明天再邀请朋友一起来玩，还有更多惊喜！', '邀请朋友来玩，有惊喜在等你！', '恭喜您，抽中一张超值奖券！奖券： ',
		...
		methods: {
			...
			gomain() {
				...
				uni.navigateTo({
					url: '/pages/selective/selective?id=296&form=draw'
				})
			},
			invitation() {
				this.$refs.popup.close()
			...
			share() {
				uni.request({
					url: 'http://39.107.114.246/ets-service/lottery/share', //仅为示例，并非真实接口地址。
					method: 'POST',
					data: {
						guid: this.guid,
						token: this.token
