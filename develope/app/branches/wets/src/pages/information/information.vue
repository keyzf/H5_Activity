    ...
    <swiper :current="tabCurrentIndex" class="swiper-box" duration="300" @change="changeTab">
      <swiper-item class="tab-content" v-for="(tabItem, tabIndex) in navList" :key="tabIndex">
        <scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
          ...
          <!-- 消息 -->
          <view class="item" v-for="(item,index) in tabItem.msg_list" :key="index" @click="onclick_f(tabItem.type,item.direction,item.title,item.pushtype,item)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //点击跳转
      onclick_f(state, direction, title, pushtype,item) {
        //type 1 物流  2 商品  0 通知 3活动 7店铺首页
        if(item.weburl){
          var number = item.weburl.indexOf("/pages/");
          var kurl = '';
          if(number > -1){
            kurl = item.weburl.slice(number)
          }else{
            kurl = item.weburl
          }
          uni.navigateTo({
            url: kurl + '?id=' + direction.split(",")[1]
          });
          return
        }
        let url = "";
