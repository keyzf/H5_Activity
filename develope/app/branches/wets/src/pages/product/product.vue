<template>
  <view class="container">
    <view id="detimal">
        ...
        <swiper indicator-dots circular="true" duration="400" @change="closevideo">
          <swiper-item class="swiper-item" v-for="(item, index) in data.picture.urlList" :key="index">
            <view class="image-wrapper">
              ...
              <image v-if="data.mainpicIcon != ''" :src="data.mainpicIcon" mode="aspectFit" class="tipimg" @click="openvideo(item.videourl)"></image>
            </view>
          </swiper-item>
        </swiper>
        <image v-if="data.activityIconSwitch == 1" :src="data.activityIcon" mode="aspectFit" class="titleicon"></image>
      </view>
      <view class="rob" v-if="data.groupbuysource == 5 || data.robstate <= 1">
        <view class="con">
          <view class="new">{{ data.newPriceRange }}</view>
          <view>
            <view class="old">{{ data.oldPriceRange }}</view>
            <text class="totalSales" v-if="data.robstate <= 1">{{ data.totalSales }}人已购买</text>
            <text class="totalSales" v-else>{{ data.groupbuy_totalnumber }}</text>
          </view>
        </view>
        <view class="time">
          <view class="tip">{{ data.groupbuy_timetext }}</view>
          <countdown :time="data.groupbuy_timestamp" @timeup="robtip(data.robstatetext)"></countdown>
        </view>
      </view>
      <view class="introduce-section">
        <view class="price-box" v-if="data.groupbuysource != 5">
        ...
        <view class="title">
          <img v-if="data.titleIcon != ''" :src="data.titleIcon" />
          <text v-if="data.maxdiscounts != '' || data.presell != ''" class="preferential">{{ data.maxdiscounts + data.presell }}</text>
          <img v-if="data.activityTitleIconSwitch == 1" :src="data.activityTitleIcon" />
          {{ data.mainheading }}
        </view>
      ...
      <view class="c-list" v-if="data.groupbuysource != 5">
      ...
      <view v-else class="introduce" @click="openedit">
        <view class="con">
          <text>玩法介绍</text>
        ...
        <view class="main">
          ...
          <view class="item x" v-if="data.groupbuysource != 5">
      ...
      <view class="c-list">
        <view v-if="data.isFictitious == 0 || tabid">
          ...
          <view class="c-row b-b" @click="switchaddress()">
            ...
            <view class="con-list clamp">
              ...
              <view>
                <text v-if="data.stockText" class="con t-r red">{{ data.stockText }}</text>
                <text>{{ data.stockAddress }}</text>
              </view>
            </view>
            <text class="yticon icon-you"></text>
        ...
        <view class="share-section">
          <view class="share-icon" :class="{ x: item.type == 2 }" v-for="(item, index) in data.after_sale_list" :key="index">{{ item.text }}</view>
        </view>
      </view>
      ...
      <view class="stores-main">
        <view class="clamp">
          <text v-for="(item, index) in companyInfos.alist" :key="index">{{ item.display }}</text>
        </view>
        <view class="lists">
    ...
    <view class="recommend">
      ...
      <view class="goods-list">
        <view class="goods-item" v-for="(item, index) in guess" :key="index" @click="navToDetailPage(item)">
          ...
          <view class="item-con">
            <view class="titles">
              <img v-if="item.titleIcon != ''" :src="item.titleIcon" />
    ...
    <view class="page-bottom" v-if="showtab && data.productkind != 4">
      ...
      <block v-else>
        <view @click="back" class="p-b-btn">
          <text class="yticon icon-gouwuche"></text>
          <text>购物车</text>
        </view>
      </block>
      <view class="action-btn-group" v-if="data.isuse != 0 && data.groupbuysource == 5">
        <button type="primary" class=" action-btn no-border buy-now-btn" @click="openpurchase">
          <view>
            <view>{{ data.newPriceRange }}</view>
            <view>单独购买</view>
          </view>
        </button>
        <button type="primary" class=" action-btn no-border add-cart-btn" @click="openpurchase">
          <view>
            <view>{{ data.groupbuyOnePrice }}</view>
            <view>发起拼团</view>
          </view>
        </button>
      </view>
      <view class="action-btn-group" v-if="data.isuse == 0 && data.stockState != 0 && data.groupbuysource == 0">
        <button type="primary" class=" action-btn no-border buy-now-btn" v-if="!data.isFictitious || data.isFictitious == 0" @click="buycard">加入购物车</button>
    ...
    <view class="popup spec" :class="specClass" @click="toggleSpec">
      ...
      <view class="layer attr-content" @click.stop="stopPrevent">
        ...
        <view v-if="data.stockState != 0">
          <view class="shop-carc">
            <text style="font-size: 28upx;flex-grow: 1;">数量</text>
            <uni-number-box
              class="step"
              :min="1"
              :max="shoppingcart.rechargetype != 0 ? shoppingcart.buylimitnumberonetime : shoppingcart.stock"
              :value="number > shoppingcart.stock ? shoppingcart.stock : number"
              :isMax="number >= (shoppingcart.rechargetype != 0 ? shoppingcart.buylimitnumberonetime : shoppingcart.stock) ? true : false"
              :isMin="number === 1"
              @eventChange="numberChange"
            ></uni-number-box>
          </view>
          <view class="shop-carc" v-if="shoppingcart.rechargetype != 0 && shoppingcart.rechargetypeinfo">
            <text style="font-size: 28upx;margin-right: 20rpx;">{{ shoppingcart.rechargetypeinfo.rechargetypeText }}</text>
            <input style="flex: 1;" type="text" v-model="daniurechargenumber" :placeholder="shoppingcart.rechargetypeinfo.rechargetypeInputTextTip" />
          </view>
          <view v-if="data.groupbuysource == 5">
            <view class="action-btn-group" style="margin: 32rpx auto 20rpx;">
              <button type="primary" class=" action-btn no-border buy-now-btn" @click="purchase(0)">单独购买</button>
              <button type="primary" class=" action-btn no-border add-cart-btn" @click="purchase(1)">拼团购买</button>
            </view>
          </view>
          <view v-else v-show="data.isuse == 0 && data.productkind != 4">
            ...
            <view class="action-btn-group" style="margin: 32rpx auto 20rpx;" v-else>
              <button type="primary" class=" action-btn no-border buy-now-btn" v-if="data.isFictitious == 0" @click="buycards">加入购物车</button>
...
export default {
  ...
  onLoad(options) {
    ...
    this.tabid = options.tabid || false;
    this.id = id;
  ...
  methods: {
    ...
    // 为你推荐
    getRecommendForYou() {
      this.$ajax.get('product/recommendForYou', {productid: this.id}).then(res => {
    ...
    getCompanyActivityAndRecommend() {
      this.$ajax.get('product/getCompanyActivityAndRecommend', {productid: this.id}).then(res => {
    ...
    load() {
      this.$ajax
        .get('product/getProductInfo2', {
          productid: this.id,
          attributes: this.specid.join(',').toString()
        })
        .then(res => {
          if (res.data.code == 0) {
    ...
    toggleSpec(i, type) {
      if (this.specClass === 'show') {
        if (this.shoppingcart.rechargetype == 1) {
          if (this.daniurechargenumber.length != 11) {
            uni.showToast({
              title: '请输入手机号码',
              icon: 'none',
              position: 'bottom',
        ...
        } else if (this.shoppingcart.rechargetype == 2) {
          if (this.daniurechargenumber.length < 5) {
            uni.showToast({
              title: '请输入正确QQ号码',
              icon: 'none',
              position: 'bottom',
    ...
    selectSpec(index, pid, kg) {
      if (!kg) {
        ...
        if (false) {
          ...
          this.shoppingcart.rechargetype = SKUResult[sl[0]].rechargetype;
          this.shoppingcart.rechargetypeinfo = SKUResult[sl[0]].rechargetypeinfo;
          this.shoppingcart.buylimitnumberonetime = SKUResult[sl[0]].buylimitnumberonetime;
          ...
          if (parkingList.length == this.shoppingcart.selectinfo.length) {
            ...
            this.shoppingcart.rechargetype = xx.rechargetype;
            this.shoppingcart.rechargetypeinfo = xx.rechargetypeinfo;
            this.shoppingcart.buylimitnumberonetime = xx.buylimitnumberonetime;
            this.keywords = xx.id;
            this.load();
          }
        }
    ...
    openactivity() {
      ...
      this.$ajax
        .get('product/getProductActivity', {
          pid: this.id
        })
        .then(res => {
          if (res.data.code == 0) {
      ...
      if (kg) {
        if(this.tabid){
          ...
          API.checkOut({
            ...
            let list = res.data.result.data[0].shopids.split(",");
            let goodsData = [];
            list.forEach(item => {
              goodsData.push({
                attr_val: item,
            ...
            uni.navigateTo({
              url: '/pages/order/createOrder?welfareid='+ this.globaldata.welfareid +'&type=cart&data=' + JSON.stringify({
                goodsData: goodsData
              })
            });
            // var url = '/pages/order/welfare_new_order?welfareid=' + this.globaldata.welfareid + '&skuinfo=' + JSON.stringify(this.skuinfo);
            // uni.navigateTo({
            //   url: url
            // });
          }).catch(err => {
          ...
          uni.navigateTo({
            url:'/pages/order/createOrder?type=direct&data=' +JSON.stringify({goodsData: goodsData,daniurechargenumber: this.daniurechargenumber,daniurechargenumbertip: this.shoppingcart.rechargetypeinfo.rechargetypeText,openGroupbuyOneTeam: this.openGroupbuyOneTeam})
          });
        }
      } else {
    ...
    purchase(type) {
      ...
      if (kg) {
        ...
        uni.navigateTo({
          url:
            '/pages/order/createOrder?type=direct&data=' +
            JSON.stringify({
              goodsData: goodsData,
              openGroupbuyOneTeam: this.openGroupbuyOneTeam
            })
        });
      } else {
        this.$api.msg('请选择正确规格');
        ...
        if(this.tabid){
          API.firstInCartInfo({}).then(res => {
            ...
            for (let i = 0; i < this.skuinfo.length; i++) {
              ...
              if (dic.skuid == this.keywords) {
                dic.number = dic.number + this.number
                hasSame = true
                break
              }
            }
    ...
    initSKU() {
      ...
      this.$ajax.get(url, data).then(res => {
        if (res.data.code == 0) {
          ...
          var i,j,skuKeys = this.getObjKeys(list.allskuinfo);
          for (i = 0; i < skuKeys.length; i++) {
            ...
            if (sku.stock > 0) {
              ...
              SKUResult[skuKeyAttrs.join(',')] = {
                ...
                pic: sku.pic,
                id: sku.id,
                rechargetype: sku.rechargetype,
                rechargetypeinfo: sku.rechargetypeinfo,
                buylimitnumberonetime: sku.buylimitnumberonetime
              };
            }
          }
          this.shoppingcart = list;
          this.$ajax.get('product/getProductInfo2', {productid: this.id}).then(res => {
              uni.hideLoading();
              if (res.data.code == 0) {
                this.data = res.data.result.data;
                this.favorite = res.data.result.data.attentionState == 0 ? false : true;
                ...
                let xh;
                if (res.data.result.data.attributeids) {
                  xh = res.data.result.data.attributeids.split(',');
                } else if (res.data.result.data.buylimitallattribute) {
                  xh = res.data.result.data.buylimitallattribute.split(',');
                } else {
                  xh = this.shoppingcart.allskuinfo[0].selectinfo.split(',');
                }
          ...
          this.$ajax
            .get('shoppingcart/getModelTipNum', {
              code: 'SHOPCART'
            })
            .then(res => {
              this.cartnumber = res.data.result.data.tipnumber;
    ...
    getTelCode() {
      this.$ajax
        .get('register/getVerificationCodeInRegister', {
