<template>
	<view class="content">
                        ...
                        <view class="con">
                            ...
                            <view class="right">
                                ...
                                <view style="overflow: hidden;">
                                    ...
                                    <text class="tips x" v-if="item.targettype != 5 && item.supportjump == 1 && tabItem.state == 1">立即使用</text>
                                </view>
		...
		<view class="coupon-item" v-for="(item, index) in couponList" :key="index" @click="clickCoupon(item)">
			<view class="con">
				<view class="left x">
					<text class="price">{{ item.discounts }}</text>
					<text>{{ item.limit }}</text>
		...
		<view class="footer"><text class="submit" @click="dklq">暂不选择</text></view>
...
export default {
	...
	onLoad(options) {
		...
		this.couponproduct = options.couponproduct;
		this.selectcoupon();
	},
	onShow(options) {
	...
	methods: {
		selectcoupon() {
			this.$ajax.get('shoppingcart/selectcoupon', { shopids: this.shopids, guid: this.guid, couponproduct: this.couponproduct }).then(res => {
				if (res.data.code == 0) {
		...
		clickCoupon(item) {
			this.$api.prePage().data.couponid = item.id;
			this.$api.prePage().data.coupon = item.discounts;
			this.$api.prePage().countnewcouponprice();
			this.$api.prePage().selectgearstepfix();
			uni.navigateBack();
		},
		// 滚动到底部加载更多
		// getCouponMore() {
		...
		//     let data = { pagesize: this.pagesize, type: navItem.state };
		...
		// // 转赠优惠券
		// donate(usercouponid) {
		//     uni.navigateTo({ url: '/pages/user/gevingcoupon?id=' + usercouponid });
		// },
		// // 立即使用
		...
		//         case 2: // 跳转搜索
		//             uni.navigateTo({
		//                 url: '/pages/product/welfare-list?targettype=2&usercouponid=' + couponInfo.usercouponid + '&targetids=' + couponInfo.keyword
		//             });
		//             break;
		//         case 3: // 跳转搜索
		//             uni.navigateTo({
		...
		// 优惠券使用说明
		getCouponIntro() {
			this.$ajax.get('user/couponIntro', { guid: this.guid }).then(res => {
				if (res.data.code == 0) {
