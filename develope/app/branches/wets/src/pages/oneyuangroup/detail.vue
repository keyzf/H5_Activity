<template>
  <view class="container">
    ...
    <view class="recommend">
      ...
      <view class="goods-list">
        <view class="goods-item" v-for="(item, index) in guess" :key="index" @click="navToDetailPage(item)">
          ...
          <view class="item-con">
            <view class="titles clamp"><text v-if="item.presell" class="presell">{{item.presell}}</text>{{ item.name }}</view>
    ...
    <view class="page-bottom" v-if="showtab && data.productkind != 4">
      ...
      <view class="action-btn-group" v-if="data.groupbuysource == 5">
      ...
      <view class="action-btn-group" v-if="data.isuse == 0 && data.stockState != 0 && data.groupbuysource == 0">
        <button type="primary" class=" action-btn no-border buy-now-btn" v-if="data.isFictitious == 0" @click="buycard">加入购物车</button>
      ...
      <view class="action-btn-group" v-else-if="data.isuse == 4"><button type="primary" class="action-btn no-border no"
          @click="toggleSpec">立即预定</button></view>
    ...
    <view class="popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="toggleSpec">
      ...
      <view class="layer attr-content" @click.stop="stopPrevent">
        ...
        <scroll-view scroll-y="true">
          <view v-for="(item, index) in shoppingcart.selectinfo" :key="index" class="attr-list">
            ...
            <view class="item-list">
              <text v-for="(childItem, childIndex) in item.attribute" :key="childIndex" class="tit" :class="{ selected: childItem.selected, selectedx: childItem.selectedx }"
        ...
        <view v-if="data.isuse == 0 && data.stockState != 0">
          ...
          <view class="shop-carc" v-if="shoppingcart.rechargetype != 0">
    ...
    <uni-popup class="restar" ref="popup" :show="register.show" type="center" :mask-click="false">
      ...
      <view class="uni-tip-content"><input class="cell-more" type="text" maxlength="11" v-model="register.telNum"
  ...
  export default {
    ...
    methods: {
      ...
      toggleMask() {
        ...
        this.$ajax.get('product/getCouponByPid', {
          productid: this.id
        }).then(res => {
          if (res.data.code == 0) {
      ...
      donate(id, index) {
        ...
        this.$ajax.get('product/getReceiveCoupon', {
          couponid: id
        }).then(res => {
          if (res.data.code == 0) {
      ...
      openparameter() {
        ...
        this.$ajax.get('product/getProductParam', {
          pid: this.id
        }).then(res => {
          if (res.data.code == 0) {
      ...
      switchaddress() {
        uni.navigateTo({
          url: '/pages/address/address?source=1&cguid=' +
        ...
        if (kg) {
          ...
          uni.navigateTo({
            url: '/pages/order/createOrder?type=direct&data=' + JSON.stringify({
              goodsData: goodsData,
              daniurechargenumber: this.daniurechargenumber,
              daniurechargenumbertip: this.shoppingcart.rechargetypeinfo.rechargetypeText,
      ...
      wantbuy() {
        ...
        if (kg) {
          let data = {
            product: '[{number:' + this.number + ',productid:' + this.id + ',allattribute:' + this.keywords +
              ',picid:' + this.shoppingcart.pic.id + ',companyid:' + this.data.companyid + '}]'
          };
      ...
      wantbuys() {
        ...
        if (kg) {
          ...
          uni.navigateTo({
            url: '/pages/product/reserveproduct?productid=' +
              this.id +
              '&keywords=' +
              this.specid +
              '&number=' +
