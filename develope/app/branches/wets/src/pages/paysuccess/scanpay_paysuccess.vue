	...
	<view class="all">
		...
		<view class="top">
			...
			<view class="successbt">
				...
				<view class="moneytext">{{qrdata.price}}</view>
				<view class="shou_title shou_text">收款方</view>
				<view class="shou_content shou_text">{{qrdata.payee}}</view>
			</view>
		</view>
		...
		<view class="goodluck">
			...
			<view v-if="lucktype == 3" class="goodlucktype1">
				...
				<view class="luckselect">
					...
					<view class="luckresult" v-if="hasselsectluck" @click="goluckresult(0)">
		...
		<view class="complete">
			...
			<text class="complete_text" @click="back">完成</text>
			
		</view>

		<!-- 为您推荐模块 -->
		<view class="recommend" v-if="showrecomend">
			<view class="title">
			...
			<view class="goods">
				<view class="goodsitem" v-for="(item,index) in goods" :key='index'>
					<goodsItemTwo :produceid='item.productid' :mainpic='item.url' :actimgurl='item.newOnShelvesProductIcon' :goodDes='item.feature_short' :tag='item.selftext' :goodTitle='item.name' :goodPrice='item.price' :bottomtag="item.activitylist" @clickGoodDetail='goodsdetail'></goodsItemTwo>
				</view>
			</view>
		</view>
	</view>
