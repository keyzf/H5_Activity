...
<template>
  <view class="all">
    ...
    <view class="top">
      ...
      <scroll-view id="tab-bar" class="scroll-h" scroll-x="true" show-scrollbar="false" :scroll-into-view="scrollInto" :style="{ 'background-color': navBarBgColor }">
    ...
    <swiper :current="tabIndex" class="swiper-box" :duration="300" @change="ontabchange" :style="{ flex: 1, height: swiperHeight + 'rpx', top: top }">
      <swiper-item class="swiper-item"><hometab :swipperDetail="swipperDetail" :fourlink="fourlink" :goodlist="goodlist" :guesslike="guesslike"></hometab></swiper-item>

      <swiper-item class="swiper-item" v-for="(tab, index1) in newsList" :key="index1"><subtab :category_guid="tab.category_guid"></subtab></swiper-item>
    </swiper>
    ...
    <uni-popup :show="showtip" :type="type" :mask-click="false">
      <view class="uni-tip">
        ...
        <view class="codes"><input class="input" maxlength="20" v-model="couponcode" placeholder="请填写邀请码" /></view>

        <view class="help" @click="helpclick">
          <image src="../../../static/fenXiao/help.png"></image>
          <view>如何获得邀请码</view>
        </view>
        <view class="connection"><view class="connectionbt" @click="commit">提交</view></view>
      </view>
      <!-- </view> -->
    </uni-popup>
  </view>
...
export default {
  ...
  methods: {
    ...
    gosearch() {
      ...
      uni.navigateTo({
        url: '../search/search?stateBarHeight=' + (parseFloat(this.stateBarHeight) - 44) + '&category_guid=' + this.current_category_guid + '&keyword=' + this.searchWord
      });
    },
    //返回
    back() {
