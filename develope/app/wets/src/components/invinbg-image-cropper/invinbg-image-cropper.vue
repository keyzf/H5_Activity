...
<script>
    //crop-fixed : 禁止缩放截图框true false   cut_scale 截图框宽高比  confirm 截取成功回调  cancel失败回调  tempFilePath 回显时的url(本地路径)
    export default {
        name: 'image-cropper',
        props: {
            cropWidth: {
        ...
        data() {
            ...
            const phoneWidth = sysInfo.windowWidth
            const pixelRatio = sysInfo.pixelRatio
            return {
                phoneWidth :phoneWidth,//手机宽度
                show: false,  
                scale : 1,
                rotate: 0,
                cropW: 0,
        ...
        methods: {
            ...
            init() {
                this.rotate = 0;
                this.cropW = this.phoneWidth - 2
                this.cropH = this.phoneWidth * this.cut_scale
                uni.showLoading({
                    title: '图片加载中...',
                })
