...
export default {
    ...
    onLoad(option) {
        this.refundtype = option.refundtype;
        this.refundcode = option.code;
    ...
    methods: {
                    ...
                    data.forEach((item, index) => {
                        ...
                        switch(item.state) {
                            ...
                            case 2: //直接退款、同意退款、拒绝退款（买家申请退货，待商家处理）
                                ...
                                // } else {
                                //     if (item.is_showbtn == 1) info.btn = [{type: 2, btnstr: '直接退款'}, {type: 3, btnstr: '同意退款'}, {type: 5, btnstr: '拒绝退款'}];
                                // }
                                break;
                            case 3: 
                                break;
                            ...
                            case 9: //直接退款（请买家退回商品，并提交物流单号）；取消退货、自己送到门店、填写物流信息（请买家退回商品，并提交物流单号）
                                if (item.refund_type_int == 0) {
                                    ...
                                    if (item.is_showbtn == 1) info.btn = [{type: 0, btnstr: '取消退货'}, {type: 7, btnstr: '送到门店'}, {type: 8, btnstr: '填写物流'}];
                                }
                                break;
        ...
        btnChange(e){
            ...
            switch(btnData.type) {
                case 0:
                    uni.showModal({
                        ...
                        success: function(res) {
                            if (res.confirm) {
                                _this.$ajax.get('com/repealRefund', {refundcode: _this.refundcode}).then(res => {
                ...
                case 3:
                    uni.navigateTo({
                        url: '/pages/shoporder/address?refundcode=' + this.refundcode
                    })
                    break;
                case 4:
                    uni.showModal({
                ...
                case 7: 
                    uni.showModal({
                        title: '提示',
                        content: '确认亲自将该商品送至卖家门店？',
                        success: res => {
                            if (res.confirm) {
                                _this.$ajax.get('com/selfDeliverToCompany', {refundcode: this.refundcode}).then(res => {
                                    _this.getRefundFlowList();
                                });
                            }
                        }
                ...
                case 8: 
                    uni.navigateTo({
                        url: '/pages/order/consignment?refundcode=' + this.refundcode
                    });
                    break;
                case 9: 
                    break;
        ...
        getRefundRule() {
            this.$ajax.get('com/getRefundRule', {}).then(res => {
                if (res.data.code == 0) {
                    uni.navigateTo({
                        url: '/pages/establishment/help?url=' + encodeURIComponent(res.data.result.data.url) + '&type=2'
                    });
                } else {
