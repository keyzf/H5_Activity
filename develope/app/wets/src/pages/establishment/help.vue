...
export default {
    ...
    onLoad(option) {
        if (option.type == 0) {
            uni.setNavigationBarTitle({ title: '帮助' });
        } else if (option.type == 1) {
            uni.setNavigationBarTitle({ title: '使用说明' });
        } else if (option.type == 2) {
            uni.setNavigationBarTitle({ title: '退款规则' });
        }else if (option.type == 3) {
            uni.setNavigationBarTitle({ title: '招商' });
        }else if (option.type == 4) {
            uni.setNavigationBarTitle({ title: '抢购规则' });
        }
        if(option.type == 0){
