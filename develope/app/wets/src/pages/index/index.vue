  ...
  <view class="container">
    <nav-bar type="transparentFixed" transparentFixedFontColor="#DDD">
      ...
      <view slot="transparentFixedTitle">
        <image class="titleimgs" src="../../static/titleimg.png" mode="aspectFit"></image>
      </view>
    ...
    <view class="carousel-section">
      ...
      <swiper class="carousel" autoplay circular indicator-dots>
        <swiper-item v-for="(item, index) in carouselList" :key="index" class="carousel-item" @click="toDetailPage(item)"><img
    ...
    <view v-for="(row, inse) in datalist" :key="inse">
      ...
      <template v-if="row.type == 22">
        <view class="f-header">
          <image :src="row.pic[0].url" :style="'width:' + row.pic[0].width + 'rpx;height:' + row.pic[0].height + 'rpx;'"></image>
        </view>
      </template>
    </view>
    <!-- 活动区 -->
    ...
    methods: {
      ...
      goenvelopes() {
        ...
        if(!userinfo.guid){
          ...
          location.href = 'http://holdsoft.holdsoft.cn/activity/20200319/share_activity.html?guid=' + userinfo.guid +'&token=' + userinfo.token + '&AppCode=WXMall'
        }
      }
    }
  };
